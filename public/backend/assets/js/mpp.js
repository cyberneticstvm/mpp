            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    $.each(JSON.parse(res.appointments), function (key, value) {
                        let cls = (value.disabled) ? `<p class="text-danger">Booked</p>` : `<div class="radio radio-primary"><input id="${value.time}" type="radio" name="radio1" value="option1"><label for="${value.time}"></label></div>`;
                        output += `<div class="card col-3 col-md-3 m-1"><div class="top-dealerbox text-center"><h6 class="mt-3">${value.time}</h6>${cls}</div></div>`
                    });
                    output += `</div></div>`;
